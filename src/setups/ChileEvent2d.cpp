...
tsunami_lab::setups::ChileEvent2d::ChileEvent2d(std::string pathBathymetry, std::string pathDisplacement, t_idx i_nx, t_idx i_ny) {
    ...
    if (l_bathy.size() != nx * ny) {
        throw std::runtime_error("ChileEvent2d: bathymetry variable has unexpected shape");
    }
    if (l_disp.size() != nx_d * ny_d) {
        throw std::runtime_error("ChileEvent2d: displacement variable has unexpected shape");
    }
...
tsunami_lab::t_real tsunami_lab::setups::ChileEvent2d::getHeight(t_real i_x, t_real i_y) const {
