...
tsunami_lab::setups::TohokuEvent2d::TohokuEvent2d(
    ...
    t_idx i_ny)
{
    ...
    if (l_bathy.size() != nx * ny) {
        throw std::runtime_error("TohokuEvent2d: bathymetry shape mismatch");
    }
    if (l_disp.size() != nx_d * ny_d) {
        throw std::runtime_error("TohokuEvent2d: displacement shape mismatch");
    }
